...
import { ConnectButton } from '@rainbow-me/rainbowkit'; // optional if you keep a quick connect in component header area

import { STAKING_ABI } from '../lib/abis/stakingAbi'; // your proxy ABI (the ABI you gave earlier)
...
export default function AfrodexStaking() {
  ...
  // IMPORTANT: Use TOKEN_ADDRESS (proxy) for calls that are exposed on the proxy (viewStakeInfoOf, stake, unstake, decimals, balanceOf, allowance, approve etc).
  // Many mistakes came from calling the implementation contract address. Ensure your src/lib/contracts exports TOKEN_ADDRESS = proxy (0x0813...)
  // and STAKING_ADDRESS is set appropriately. If staking functions live on proxy itself you can set STAKING_ADDRESS === TOKEN_ADDRESS.

  const fetchOnChain = useCallback(async () => {
    if (!publicClient) return;
    try {
      // read decimals from TOKEN proxy first (so we format rewardRate correctly)
      ...
      // allowance: allow spender = STAKING_ADDRESS (if staking requires a token approval) — some deployments require approving the staking contract (which may be the proxy itself)
      // We'll request allowance(owner, spender) using STAKING_ADDRESS. Confirm in your lib/contracts whether STAKING_ADDRESS === TOKEN_ADDRESS (proxy) or different.
      ...
      // stakeInfo: call viewStakeInfoOf on the PROXY (or staking entrypoint) — ensure the ABI includes viewStakeInfoOf
      if (address) {
        const stakeInfo = await readContractSafe(publicClient, {
          address: TOKEN_ADDRESS,          // <-- use TOKEN_ADDRESS (proxy) because viewStakeInfoOf is exposed there
          abi: STAKING_ABI,
      ...
      // rewardRate stored as uint scaled by token decimals — format using the token decimals we just read
      setRewardRate(rewardRateRaw ? Number(formatUnits(rewardRateRaw, d)) : 0);
      setBonusRate(bonusRateRaw ? Number(formatUnits(bonusRateRaw, d)) : 0);
  ...
  // ----- WRITE helpers (robust)
  // doApprove: approve spender = STAKING_ADDRESS (some deployments require approving staking contract)
  const doApprove = async (humanAmount) => {
    if (!walletClient || !isConnected) { showAlert('Connect wallet'); return; }
    setLoading(true);
    try {
      const raw = toRaw(humanAmount);
      // write to TOKEN_ADDRESS (it's the token contract)
      ...
      // try to wait for confirmation (best-effort)
      try {
  ...
  // doStake: attempt to call stake on the PROXY (i.e. TOKEN_ADDRESS). If it reverts, attempt approve -> stake fallback.
  const doStake = async (humanAmount) => {
    ...
    try {
      ...
      // attempt to call stake on TOKEN_ADDRESS (proxy)
      try {
        const tx = await writeContractSafe(walletClient, {
          address: TOKEN_ADDRESS,         // stake function is on proxy
          abi: STAKING_ABI,
          functionName: 'stake',
          args: [raw],
        });
        ...
        // if stake reverted, try approve flow and then try alternative depositToken if available
        console.warn('Direct stake failed, attempting approve fallback', errStake);
      }

      // fallback: ensure allowance for STAKING_ADDRESS (owner approved spender), then call stake on proxy or depositToken on possible staking implementation.
      ...
      if ((allowRaw ?? 0n) < raw) {
        ...
        await doApprove(humanAmount);
      }

      // try second attempt: stake on TOKEN_ADDRESS or depositToken on STAKING_ADDRESS
      const tryFns = [
        { addr: TOKEN_ADDRESS, fn: 'stake', args: [raw] }, // preferred
        { addr: STAKING_ADDRESS, fn: 'depositToken', args: [TOKEN_ADDRESS, raw] }, // common fallback pattern
      ];
      ...
      if (!done) throw new Error('Stake failed after fallback attempts');
      showAlert('Stake confirmed (fallback path)');
      setAmount('');
      await fetchOnChain();
  ...
  const doUnstake = async (humanAmount) => {
    ...
    try {
      const raw = toRaw(humanAmount);
      // call unstake on proxy (TOKEN_ADDRESS) or on STAKING_ADDRESS if not found
      try {
        const tx = await writeContractSafe(walletClient, {
          address: TOKEN_ADDRESS,
          abi: STAKING_ABI,
  ...
  // Claim: try many variants; if none exist attempt tiny unstake-restake fallback
  const doClaim = async () => {
    if (!walletClient || !isConnected) { showAlert('Connect wallet'); return; }
    setLoading(true); setTxHash(null);
    const candidates = [
      { addr: TOKEN_ADDRESS, fn: 'claim', args: [] },
      { addr: TOKEN_ADDRESS, fn: 'claimRewards', args: [] },
      { addr: TOKEN_ADDRESS, fn: 'withdrawReward', args: [] },
      { addr: TOKEN_ADDRESS, fn: 'getReward', args: [] },
      { addr: STAKING_ADDRESS, fn: 'claim', args: [] },
    ];
  ...
  const APR = useMemo(() => (dailyRate + monthlyBonusRate * 12) * 365 * 100, [dailyRate, monthlyBonusRate]);

  // calculator states
  const [calcDays, setCalcDays] = useState(365);
  const [calcAmt, setCalcAmt] = useState('');
  useEffect(() => { setCalcAmt(stakedBalance || ''); }, [stakedBalance]);

  const calcResult = useMemo(() => calcRewardsFor(Number(calcAmt || stakedBalance || 0), Number(calcDays || 0)), [calcAmt, calcDays, stakedBalance, dailyRate, monthlyBonusRate]);
  ...
  return (
    <div className="min-h-screen w-full bg-black text-white antialiased">
      {/* Do NOT render ConnectHeader here — it should be rendered by pages/index.js to avoid duplication */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-gray-800 p-4 rounded-xl border border-orange-600/20 flex flex-col justify-center items-start">
              ...
              <div className="text-2xl font-bold text-orange-300">
                {APR.toFixed(2)}% <span className="text-sm text-gray-400">(if staked for {calcDays} days)</span>
              </div>
