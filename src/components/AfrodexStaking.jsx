...
// FIXED: Token Analytics chart restored
// FIXED: Hourly, Daily, Monthly, Yearly projections
// FIXED: USD values showing
// FIXED: Badge tier correctly calculated
// FIXED: Passes badge data to child components
'use client';
...
// Badge tier thresholds in human-readable format
...
// Helper function to get badge tier from staked amount
...
// Token Analytics Donut Chart Component
function TokenAnalyticsChart({ currentSupply, rewardsMinted, maxSupply }) {
  ...
  function prettyNumber(num, precision = 2) {
    ...
    <motion.div className="bg-gray-900 p-6 rounded-3xl border border-orange-600/10" whileHover={{ boxShadow: '0 0 18px rgba(255,140,0,0.12)' }}>
        ...
        <div className="relative">
          <svg width="200" height="200" viewBox="0 0 200 200">
            ...
            <circle
              ...
              fill="none" stroke="#f97316" strokeWidth="24"
              strokeDasharray={`${(unmintedPct / 100) * circumference} ${circumference}`}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <div className="text-xl font-bold text-white">{prettyNumber(max)}</div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-semibold text-white">{prettyNumber(current)}</span>
        </div>
        
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-green-500"></div>
            <span className="text-sm text-gray-400">Rewards Minted:</span>
          </div>
          <span className="text-sm font-semibold text-green-400">{prettyNumber(minted)}</span>
        </div>
        
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-orange-500"></div>
            <span className="text-sm text-gray-400">Un-minted:</span>
          </div>
          <span className="text-sm font-semibold text-orange-400">{prettyNumber(unminted)}</span>
        </div>
      </div>
    </motion.div>
  );
...
export default function AfrodexStaking() {
  ...
  function prettyNumber(humanStr, precision = 2) {
    ...
    if (absN >= 1e12) return (n / 1e12).toFixed(precision) + 'T';
    if (absN >= 1e9) return (n / 1e9).toFixed(precision) + 'B';
    if (absN >= 1e6) return (n / 1e6).toFixed(precision) + 'M';
    if (absN >= 1e3) return (n / 1e3).toFixed(precision) + 'K';
  ...
  const fetchOnChain = useCallback(async () => {
    ...
    try {
      ...
      const decRaw = await readContractSafe(publicClient, {
        address: TOKEN_ADDRESS, abi: AFROX_PROXY_ABI, functionName: 'decimals', args: [],
      });
      ...
      if (address) {
        const walletBal = await readContractSafe(publicClient, {
          address: TOKEN_ADDRESS, abi: AFROX_PROXY_ABI, functionName: 'balanceOf', args: [address],
        });
        setWalletBalance(walletBal !== null ? toHuman(walletBal) : '0');

        const stakeInfo = await readContractSafe(publicClient, {
          address: STAKING_ADDRESS, abi: STAKING_ABI, functionName: 'viewStakeInfoOf', args: [address],
        });
  ...
  async function doApprove(amountHuman) {
    try {
      ...
      const tx = await writeContractSafe(ensureClient(), {
        address: TOKEN_ADDRESS, abi: AFROX_PROXY_ABI, functionName: 'approve', args: [STAKING_ADDRESS, raw],
      });
  ...
  async function doStake(humanAmount) {
    try {
      ...
      const tx = await writeContractSafe(ensureClient(), {
        address: STAKING_ADDRESS, abi: STAKING_ABI, functionName: 'stake', args: [raw],
      });
      setTxHash(tx?.hash ?? null);
  ...
  async function doUnstake(humanAmount) {
    try {
      ...
      const raw = toRaw(humanAmount);
      const tx = await writeContractSafe(ensureClient(), {
        address: STAKING_ADDRESS, abi: STAKING_ABI, functionName: 'unstake', args: [raw],
      });
      setTxHash(tx?.hash ?? null);
          ...
          <>
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              <motion.div className="bg-gray-900 p-4 rounded-2xl border border-orange-600/10" whileHover={cardGlow}>
                ...
                <div className="text-xs text-gray-400 mt-1">Days staked: {stakedDays}</div>
              </motion.div>
            ...
            <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              ...
              <motion.div className="bg-gray-900 p-6 rounded-3xl" whileHover={{ scale: 1.01 }}>
                ...
                {txHash && <div className="mt-4 text-xs text-gray-400">Tx: <span className="text-orange-200 break-all">{txHash}</span></div>}
              </motion.div>
            </section>
