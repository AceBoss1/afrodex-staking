...
const useAfrodexTokenBalance = (address?: Address) => {
  return useContractRead({
    address: AFRODEX_TOKEN_ADDRESS as Address,
...
const useTokenAllowance = (address?: Address) => {
  return useContractRead({
    ...
    args: [address as Address, STAKING_CONTRACT_ADDRESS as Address],
    enabled: !!address,
    watch: true,
  });
};
...
export default function StakingComponent() {
  ...
  const { data: tokenBalance, refetch: refetchTokenBalance } = useAfrodexTokenBalance(userAddress as Address);
  const { data: allowance, refetch: refetchAllowance } = useTokenAllowance(userAddress as Address);
  const { data: stakeInfo, refetch: refetchStakeInfo } = useStakeInfo(userAddress as Address);
  ...
  const needsApproval = useMemo(() => {
    // 10M tokens is a safe, large allowance threshold
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      // Calling writeContract with full parameters (address, abi, functionName, args)
      await writeApprove({
        address: AFRODEX_TOKEN_ADDRESS as Address,
        abi: AFRODEX_TOKEN_ABI,
        functionName: 'approve',
        args: [
          STAKING_CONTRACT_ADDRESS as Address,
          maxApproval,
        ],
      });
    } catch (e) {
